 ...
 */
#define ESP_LV_ADAPTER_TE_SYNC_DISABLED() (esp_lv_adapter_te_sync_config_t){.gpio_num = -1}
...
#define ESP_LV_ADAPTER_DISPLAY_PROFILE_BASE_CONFIG(_hor_res, _ver_res, _rotation) \
    .rotation = (_rotation), .hor_res = (_hor_res), .ver_res = (_ver_res),
...
#define ESP_LV_ADAPTER_DISPLAY_PROFILE_MIPI_DEFAULT_CONFIG(_hor_res, _ver_res, _rotation)                             \
    ...
    ESP_LV_ADAPTER_DISPLAY_PROFILE_BASE_CONFIG(_hor_res, _ver_res, _rotation).buffer_height = 50, .use_psram = false, \
    .enable_ppa_accel = false, .require_double_buffer = false, .mono_layout = ESP_LV_ADAPTER_MONO_LAYOUT_NONE
...
#define ESP_LV_ADAPTER_DISPLAY_PROFILE_SPI_WITH_PSRAM_BASE_CONFIG(_hor_res, _ver_res, _rotation)        \
    ...
    ESP_LV_ADAPTER_DISPLAY_PROFILE_BASE_CONFIG(_hor_res, _ver_res, _rotation).buffer_height = _ver_res, \
...
#define ESP_LV_ADAPTER_DISPLAY_PROFILE_SPI_WITH_PSRAM_DEFAULT_CONFIG(_hor_res, _ver_res, _rotation) \
    ESP_LV_ADAPTER_DISPLAY_PROFILE_SPI_WITH_PSRAM_BASE_CONFIG(_hor_res, _ver_res, _rotation),       \
        .require_double_buffer = true, .mono_layout = ESP_LV_ADAPTER_MONO_LAYOUT_NONE

#define ESP_LV_ADAPTER_DISPLAY_PROFILE_SPI_WITH_PSRAM_TE_DEFAULT_CONFIG(_hor_res, _ver_res, _rotation) \
    ESP_LV_ADAPTER_DISPLAY_PROFILE_SPI_WITH_PSRAM_BASE_CONFIG(_hor_res, _ver_res, _rotation),          \
        .require_double_buffer = false, .mono_layout = ESP_LV_ADAPTER_MONO_LAYOUT_NONE
...
#define ESP_LV_ADAPTER_DISPLAY_PROFILE_SPI_WITHOUT_PSRAM_DEFAULT_CONFIG(_hor_res, _ver_res, _rotation)                \
    .interface = ESP_LV_ADAPTER_PANEL_IF_OTHER,                                                                       \
    ESP_LV_ADAPTER_DISPLAY_PROFILE_BASE_CONFIG(_hor_res, _ver_res, _rotation).buffer_height = 10, .use_psram = false, \
    .enable_ppa_accel = false, .require_double_buffer = false, .mono_layout = ESP_LV_ADAPTER_MONO_LAYOUT_NONE

#define ESP_LV_ADAPTER_DISPLAY_PROFILE_MONO_DEFAULT_CONFIG(_hor_res, _ver_res, _rotation, _mono_layout)   \
    .interface = ESP_LV_ADAPTER_PANEL_IF_OTHER,                                                           \
    ESP_LV_ADAPTER_DISPLAY_PROFILE_BASE_CONFIG(_hor_res, _ver_res, _rotation).buffer_height = (_ver_res), \
    .use_psram = false, .enable_ppa_accel = false, .require_double_buffer = false, .mono_layout = (_mono_layout)
...
#define ESP_LV_ADAPTER_DISPLAY_MIPI_DEFAULT_CONFIG(_panel, _panel_io, _hor_res, _ver_res, _rotation)                 \
    ESP_LV_ADAPTER_DISPLAY_CONFIG(_panel, _panel_io,                                                                 \
                                  ...
                                  ESP_LV_ADAPTER_TEAR_AVOID_MODE_DEFAULT_MIPI_DSI, ESP_LV_ADAPTER_TE_SYNC_DISABLED())
...
#define ESP_LV_ADAPTER_DISPLAY_SPI_WITH_PSRAM_DEFAULT_CONFIG(_panel, _panel_io, _hor_res, _ver_res, _rotation) \
    ESP_LV_ADAPTER_DISPLAY_CONFIG(                                                                             \
        ...
        ESP_LV_ADAPTER_TEAR_AVOID_MODE_DEFAULT, ESP_LV_ADAPTER_TE_SYNC_DISABLED())
 ...
 */
#define ESP_LV_ADAPTER_DISPLAY_SPI_WITH_PSRAM_TE_DEFAULT_CONFIG(_panel, _panel_io, _hor_res, _ver_res, _rotation, \
                                                                _te_gpio, _bus_freq, _data_lines, _bpp)           \
    ESP_LV_ADAPTER_DISPLAY_CONFIG(                                                                                \
        _panel, _panel_io,                                                                                        \
 ...
 *      - ESP_ERR_NOT_FOUND: Display not found in registered list
 */
esp_err_t esp_lv_adapter_set_area_rounder_cb(lv_display_t* disp, void (*rounder_cb)(lv_area_t* area, void* user_data),
                                             void* user_data);
...
typedef struct {
     ...
     */
    esp_err_t (*custom_draw_bitmap)(lv_display_t* disp, esp_lcd_panel_handle_t panel, int x_start, int y_start,
                                    int x_end, int y_end, const void* color_map, void* user_ctx);
...
esp_err_t esp_lv_adapter_set_draw_bitmap_callbacks(lv_display_t* disp,
                                                   const esp_lv_adapter_draw_bitmap_callbacks_t* cbs, void* user_ctx);
...
typedef struct {
    ...
    uint64_t panel_submit_bytes;
    uint64_t panel_submit_elapsed_us;
    uint64_t panel_vsync_wait_calls;
    uint64_t panel_vsync_wait_elapsed_us;
    uint64_t rgb565_wire_cpu_count;
    uint64_t rgb565_wire_cpu_pixels;
    uint64_t rgb565_wire_cpu_us;
